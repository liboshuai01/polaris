...
public class PermissionDataUtil {
     ...
     * @param permission
     */
    public static SysPermissionEntity intelligentProcessData(SysPermissionEntity permission) {
        if (permission == null) {
            return null;
        }
     ...
     * @param metaList
     */
    public static void addIndexPage(List<SysPermissionEntity> metaList) {
        boolean hasIndexMenu = false;
        for (SysPermissionEntity sysPermission : metaList) {
        ...
        if(!hasIndexMenu) {
            metaList.add(0,new SysPermissionEntity(true));
        }
    }
     ...
     * @return
     */
    public static boolean hasIndexPage(List<SysPermissionEntity> metaList){
        boolean hasIndexMenu = false;
        for (SysPermissionEntity sysPermission : metaList) {
            if("首页".equals(sysPermission.getName())) {
                hasIndexMenu = true;
