...
//                && !CollectionUtils.isEmpty(queryAllDictItems.getData())) {
...
//        if (Objects.isNull(userByName) || !userByName.isSuccess() || Objects.isNull(userByName.getData())) {
//            return ResponseResult.fail("不存在此用户");
//        }
//        SysUserDTO sysUserDTO = userByName.getData();
...
//            }
//            if (CollectionUtils.isEmpty(queryEffectiveTenant.getData())) {
//                return ResponseResult.fail("与该用户关联的租户均已被冻结，无法登录！");
//            }
//            List<SysTenantDTO> sysTenantDTOS = queryEffectiveTenant.getData();
...
//        } else {
//            List<SysDepartDTO> sysDepartDTOS = queryUserDeparts.getData();
...
//        }
    ...
    private Map<String, List<SysDictItemVO>> sysDictDTOMap2VOMap(ResponseResult<Map<String, List<SysDictItemDTO>>> queryAllDictItems) {
        ...
        for (Map.Entry<String, List<SysDictItemDTO>> entry:
                queryAllDictItems.getData().entrySet()){
