...
public class ResourceUtil {
     ...
     * 枚举字典数据
     */
    private final static Map<String, List<SysDictItemVO>> enumDictData = new HashMap<>(5);
     ...
     * @return
     */
    public static Map<String, List<SysDictItemVO>> getEnumDictData(){
            ...
            for (Resource resource : resources) {
                ...
                if (enumDict != null) {
                    ...
                    if(oConvertUtils.isNotEmpty(key)){
                        List<SysDictItemVO> list = (List<SysDictItemVO>) clazz.getDeclaredMethod(METHOD_NAME).invoke(null);
     ...
     * @return
     */
    public static Map<String, List<SysDictItemVO>> queryManyDictByKeys(List<String> dictCodeList, List<String> keys){
        if(enumDictData.keySet().size()==0){
            getEnumDictData();
        }
        Map<String, List<SysDictItemVO>> map = new HashMap<>();
        for (String code : enumDictData.keySet()) {
            if(dictCodeList.indexOf(code)>=0){
                List<SysDictItemVO> dictItemList = enumDictData.get(code);
                for(SysDictItemVO dm: dictItemList){
                    String value = dm.getValue();
                    if(keys.indexOf(value)>=0){
                        List<SysDictItemVO> list = new ArrayList<>();
                        list.add(new SysDictItemVO(value, dm.getText()));
                        map.put(code,list);
                        break;
                    }
                }
